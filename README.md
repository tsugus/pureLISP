...
純LISP のインタプリタです。実際に純LISP の腸循環評価器を動かせるものを目指しました（ただし、シミュレートされる LISP とする LISP とで細部の形式が微妙に異なりますが）。
...
DOSBOX などの DOS/V マシン・エミュレータで Turbo C 2.0 を用いてコンバイルすることもできます。その場合は、できるだけ大きなスタックサイズを指定し、総セル数とソフトウェア・スタック（これはコンパイラとは無関係に自前で用意したもの）のサイズを減らす必要があります。
...
を MAIN.C 内に付け加えることできギリギリまで増やせます。

総セル数とソフトウェア・スタックのサイズは、LISP.H の中の define 文で定義されているので、

    #define CELLS_SIZE 0x3800
    ...
    #define STACK_SIZE 0x2000

などと数値を変更してください。ただし、これらの数値はあくまで一例です。あと、改行コードを LF から CRLF に変更する必要があります。
...
すなわち、数アトムを持たない純LISP において、自然数をリストの長さで代用する方法です。
...
このようなタイプの「数」は実用的ではありませんが、純LISP で自然数を原理的に実現する最も簡単な方法です。
